...
<body>
  ...
  <option value="URINE">
    ...
    <form onsubmit="return check();" action="booktest" method="post" >
	...
	<table id="cart" class="table table-hover table-condensed">
    				<thead>
						<tr>
							...
							<th style="width:35%" class="text-center">Subtotal</th>
					...
					<tbody>
						<tr>
							<td data-th="Product">
								<div class="row">
									<div class="col-sm-10">
										<h4 class="nomargin"><label>${tname}</label></h4>
										<p>Customer state: <b>${tTime}</b></p>
										<c:if test="${tTime == 'Fasting'}"> 
										<c:set var = "des" scope = "session" value = "${'It is important for patients to avoid eating, drinking anything for 8 to 10 hours'}"/>
										<p><c:out value = "${des}"/></p>
										</c:if>
										<c:if test="${tTime == 'Postprandial'}"> 
										<c:set var = "des" scope = "session" value = "${'This test is done to see how your body responds to sugar after you eat a meal. So you should have your meal 1-2 hours before test.'}"/>
										<p><c:out value = "${des}"/></p>
										</c:if>
													</div>
								</div>
							</td>
							<td data-th="Price">₹ ${tcost}</td>
							<td data-th="Quantity">
								<input name="date" type="date" id="date" oninput="gen()" class="form-control text-center" required >
							</td>
							<td data-th="Subtotal" class="text-center">₹ ${tcost}</td>
							<td><input name="ideal" placeholder="Auto-generated" type="text" id="ideal" name="ideal" size="10" readonly/></td>
						</tr>
					</tbody>
					<tfoot>
						<tr>
							...
							<td class="hidden-xs text-center"><strong>Total: ₹${tcost}</strong></td>
...
function gen() {
	...
	for (var i = 0; i <= passwordLength; i++) {
	  ...
	  pass += chars.substring(randomNumber, randomNumber +1);
	       ...
	       var id = "REQ"+slice.toUpperCase()+pass;
	       document.getElementById("ideal").value = id;
	}
function check(){
